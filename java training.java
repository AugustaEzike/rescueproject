...
/*In general, we recommend using Swing timers rather than general-purpose timers for 
 * GUI-related tasks because Swing timers all share the same, pre-existing timer thread 
 * and the GUI-related task automatically executes on the event-dispatch thread. 
 * However, you might use a general-purpose timer if you don't plan on touching the GUI 
 * from the timer, or need to perform lengthy processing.
...
public class GamePanel extends JPanel implements ActionListener {
		...
		if (running) {
			...
			for (int i = 0; i< bodyParts; i++) {
				if(i == 0) {
					...
					g.setColor(new Color(45, 180, 0)); //hex value
	...
	//populate with apple to score point
	...
	//method to eat apples and win a point
